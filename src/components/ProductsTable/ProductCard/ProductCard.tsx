...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div
      className={`${styles.productCard} flex flex-col rounded-2xl h-[15rem] md:h-[21rem] px-2 md:px-4 ${dense ? "py-2" : "py-3 md:py-3"} max-w-[30rem] justify-between ${className || ""}`}
      style={{ ...(cardStyle || {}), ...(height !== undefined ? { height: typeof height === "number" ? `${height}px` : height } : {}) }}
    >
      {/* header */}
      <div className={`relative ${dense ? "mb-1" : "mb-1 md:mb-4"} flex w-full justify-center items-center`}>
        <div
          className={`${styles.productCardScore} absolute top-0 left-0 flex items-center justify-center w-8 h-8`}
        >
          <HeartIcon
            className={`${styles.productCardHeart} ${dense ? "w-[16px] h-[16px]" : "w-[20px] h-[18px]"} cursor-pointer`}
          />
        </div>
        ...
        <Image
          ...
          width={134}
          height={142}
          className={`w-auto  ${dense ? "max-h-[72px]" : "max-h-[84px] md:max-h-[125px] lg:max-h-[140px]"}`}
        />
      ...
      {/* body */}
      <div className={` mb-0 md:mb-1 flex ${dense ? "gap-1" : "gap-2"} items-center`}>
      ...
      <div className="flex gap-2 items-center w-full">
        <div className="relative text-center max-h-[54px] w-full flex flex-row justify-between">
          ...
          <div
            className={`${styles.marketCardHeadPercent} ${percentClass} absolute ${dense ? "bottom-3 left-14" : "bottom-3 left-16"} flex items-center justify-center gap-1 min-w-14  font-bold`}
          >
            {ArrowIcon && <ArrowIcon />}
            {changePercent} %
          </div>
